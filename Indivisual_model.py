...
plt.title('NJ COVID19 Spread Prediction with LSTM')
plt.xlabel('Days since First Case')
plt.ylabel('Number of CVID19 Cases')
plt.legend()
plt.savefig('Figures/NJ_Indivisual_State_10_days_prediction.png')
...
day=list(range(c.shape[0]))
#%%
import pandas
df = pandas.DataFrame(data={"day": day,"actual": c, "testPredict": b})
df.to_csv("NJ_Data.csv", sep=',',index=False)
